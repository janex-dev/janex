...
fn manifest_continuations_join_before_utf8_decoding_and_sections_merge() {
    ...
    let manifest = Manifest::parse(bytes, Limits::default()).unwrap();
    ...
    for invalid in [
        ...
        assert!(
            Manifest::parse(invalid, Limits::default()).is_err(),
            "{invalid:?}"
        );
    }
}
...
fn zip64_end_records_are_bounded_before_archive_index_allocation() {
    ...
    for extension_size in [0, 70000] {
        ...
        assert!(
            matches!(error, janex_core::Error::Format(ref error) if error.kind() == janex_format::ErrorKind::Limit)
        );
    }
}
