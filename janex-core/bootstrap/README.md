...
`Bootstrap.class` is compiled for Java 8 and embedded in `janex-core`. Building Janex does not
require a JDK. Rebuild it after editing the source with JDK 25:

```text
python janex-core/bootstrap/build.py
python janex-core/bootstrap/build.py --check
```
