...
fn trusted_metadata_without_complete_content_coverage_cannot_execute() {
    ...
    let bytes = writer
        .finish_with(Value::empty_map(), 3, |input| {
            Ok::<_, janex_host::Error>(cms::sign(input, &[&signer], SystemTime::now())?)
        })
        .unwrap();
    ...
    assert!(matches!(&error, janex_host::Error::Trust(_)));
...
fn openpgp_loader_requests_only_the_selected_keys_password_once() {
    let directory =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../janex-signature/tests/fixtures/openpgp");
    let mut requests = 0;
