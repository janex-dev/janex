...
public final class ModuleSupport {
    ...
    public static void initialize(ResourceLoader loader) throws Exception {
        ...
        String mainModule;
        String mainClass;
        try (DataInputStream input = new DataInputStream(ModuleSupport.class.getResourceAsStream("options.bin"))) {
            mainModule = text(input); mainClass = text(input);
            int count = input.readInt();
            if (count < 0) throw new IOException("Invalid JVM option count");
            for (int i = 0; i < count; i++) options.add(text(input));
            if (input.read() != -1) throw new IOException("Trailing JVM option data");
        }
        Finder finder = new Finder(loader);
        Map<String, ModuleDescriptor> available = new HashMap<String, ModuleDescriptor>();
        for (ModuleReference reference : ModuleFinder.ofSystem().findAll()) available.put(reference.descriptor().name(), reference.descriptor());
        for (ModuleReference reference : finder.findAll()) available.put(reference.descriptor().name(), reference.descriptor());
        for (Map.Entry<String, String> requirement : loader.index.requirements.entrySet()) {
            ...
            if (descriptor == null) throw new FindException("Required module is unavailable: " + requirement.getKey());
        ...
        if (!mainModule.isEmpty()) roots.add(mainModule);
        for (int i = 0; i < options.size(); i++) {
            String option = options.get(i);
            if (option.equals("--add-modules")) option += "=" + argument(options, ++i);
            if (option.startsWith("--add-modules=")) {
                for (String name : option.substring(14).split(",", -1)) {
                    if (name.equals("ALL-SYSTEM")) {
                        for (ModuleReference system : ModuleFinder.ofSystem().findAll()) loader.systemModules.add(system.descriptor().name());
                    }
                    if (!name.equals("ALL-MODULE-PATH") && !name.equals("ALL-SYSTEM") && !name.equals("ALL-DEFAULT")) roots.add(name);
                }
            }
        }
        for (ModuleReference reference : finder.findAll()) {
            for (ModuleDescriptor.Requires requirement : reference.descriptor().requires()) {
                if (!requirement.modifiers().contains(ModuleDescriptor.Requires.Modifier.STATIC)
                        && ModuleFinder.ofSystem().find(requirement.name()).isPresent()) loader.systemModules.add(requirement.name());
            }
        }
        for (String root : roots) if (ModuleFinder.ofSystem().find(root).isPresent()) loader.systemModules.add(root);
        ...
        if (!mainModule.isEmpty()) {
            ...
            if (dot > 0) access("addOpens", module, entry.substring(0, dot), Bootstrap.class.getModule());
        }
    }

    /// Returns a required option operand.
    private static String argument(List<String> options, int index) {
        if (index >= options.size()) throw new IllegalArgumentException("Missing module option operand");
    ...
    private static void applyOptions(ModuleLayer layer, List<String> options) throws Exception {
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            if (!accessOptions.contains(key)) continue;
            ...
            if (key.equals("--enable-native-access")) {
                for (String name : value.split(",", -1)) {
                    if (name.equals("ALL-UNNAMED")) continue;
                    Method enable = Module.class.getDeclaredMethod("implAddEnableNativeAccess");
                    enable.setAccessible(true); enable.invoke(module(layer, name));
                }
                continue;
            }
            int split = value.indexOf('=');
            if (split <= 0 || split == value.length() - 1) throw new IllegalArgumentException("Invalid module option: " + option);
            ...
            if (!operation.equals("addReads") && (slash <= 0 || slash == source.length() - 1)) throw new IllegalArgumentException("Missing module package: " + source);
            Module from = module(layer, slash < 0 ? source : source.substring(0, slash));
            String pkg = slash < 0 ? null : source.substring(slash + 1);
            if (pkg != null && !from.getPackages().contains(pkg)) throw new IllegalArgumentException("Unknown module package: " + source);
            for (String target : value.substring(split + 1).split(",", -1)) {
                if (target.equals("ALL-UNNAMED")) access(operation + "AllUnnamed", from, pkg, null);
                else access(operation, from, pkg, module(layer, target));
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        try (ResourceLoader loader = new ResourceLoader(ClassLoader.getSystemClassLoader())) {
            for (String name : loader.systemModules) { System.out.write(name.getBytes(StandardCharsets.UTF_8)); System.out.write('\n'); }
        }
    }
    ...
    private static String text(DataInputStream input) throws IOException {
        int count = input.readInt(); if (count < 0) throw new IOException("Invalid option string length");
        char[] value = new char[count]; for (int i = 0; i < count; i++) value[i] = input.readChar(); return new String(value);
    }
    ...
    private static final class Finder implements ModuleFinder {
        ...
        Finder(ResourceLoader loader) throws IOException {
            for (ResourceLoader.Root root : loader.allRoots) {
                ...
                if (ModuleLayer.boot().findModule(name).isPresent()) throw new FindException("Module shadows a system module: " + name);
                if (references.put(name, new Reference(root, descriptor)) != null) throw new FindException("Duplicate module: " + name);
                loader.moduleRoots.put(name, root);
                for (String pkg : descriptor.packages()) if (loader.modulePackages.put(pkg, root) != null) throw new LayerInstantiationException("Split module package: " + pkg);
            }
        }
        ...
        private static Set<String> packages(ResourceLoader.Root root) {
            ...
            for (String path : root.root.files.keySet()) {
                if (!path.endsWith(".class") || path.equals("module-info.class") || path.startsWith("META-INF/")) continue;
                int slash = path.lastIndexOf('/');
                if (slash < 0) throw new InvalidModuleDescriptorException("Unnamed package in module: " + path);
        ...
        private static ModuleDescriptor automatic(ResourceLoader.Root root, Set<String> packages) throws IOException {
            ...
            if (version.find()) { rawVersion = stem.substring(version.start() + 1); stem = stem.substring(0, version.start()); }
            String name = root.manifest == null ? null : root.manifest.getMainAttributes().getValue("Automatic-Module-Name");
            if (name == null) name = stem.replaceAll("[^A-Za-z0-9]", ".").replaceAll("\\.+", ".").replaceAll("^\\.|\\.$", "");
            ModuleDescriptor.Builder builder = ModuleDescriptor.newAutomaticModule(name).packages(packages);
            if (rawVersion != null) try { builder.version(rawVersion); } catch (IllegalArgumentException unparseable) { /* Optional filename version. */ }
            for (Map.Entry<String, ResourceIndex.Resource> entry : root.root.files.entrySet()) {
                String prefix = "META-INF/services/";
                if (!entry.getKey().startsWith(prefix) || entry.getValue().id == -1) continue;
                ...
                try (BufferedReader input = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(entry.getValue().read()), StandardCharsets.UTF_8.newDecoder()))) {
                    ...
                    while ((line = input.readLine()) != null) {
                        ...
                        if (dot < 0 || !packages.contains(line.substring(0, dot))) throw new InvalidModuleDescriptorException("Provider is not in module: " + line);
                        if (!providers.contains(line)) providers.add(line);
                    }
                }
                if (!providers.isEmpty()) builder.provides(service, providers);
            }
            String main = root.manifest == null ? null : root.manifest.getMainAttributes().getValue(Attributes.Name.MAIN_CLASS);
            if (main != null) {
                main = main.replace('/', '.'); int dot = main.lastIndexOf('.');
                if (dot > 0 && packages.contains(main.substring(0, dot))) builder.mainClass(main);
            }
            return builder.build();
        }
        /// Returns the named indexed module, if present.
        @Override public Optional<ModuleReference> find(String name) { return Optional.ofNullable(references.get(Objects.requireNonNull(name))); }
        /// Returns a stable immutable set of all indexed module references.
        @Override public Set<ModuleReference> findAll() { return Collections.unmodifiableSet(new LinkedHashSet<ModuleReference>(references.values())); }
    }
    ...
    private static final class Reference extends ModuleReference {
        ...
        Reference(ResourceLoader.Root root, ModuleDescriptor descriptor) { super(descriptor, URI.create(root.base.toString())); this.root = root; }
        /// Returns an independently closeable module reader.
        @Override public ModuleReader open() { return new Reader(root); }
    }
    ...
    private static final class Reader implements ModuleReader {
        ...
        Reader(ResourceLoader.Root root) { this.root = root; }
        /// Rejects operations after reader closure.
        private void check() throws IOException { if (closed.get() || root.root.isClosed()) throw new IOException("Module reader is closed"); }
        /// Rejects an empty name rather than exposing the module root as an unnamed resource.
        private ResourceIndex.Resource resource(String name) { return Objects.requireNonNull(name).isEmpty() ? null : root.resource(name); }
        /// Returns the URI of an existing exact resource.
        @Override public Optional<URI> find(String name) throws IOException { check(); return resource(name) != null ? Optional.of(URI.create(root.url(name).toString())) : Optional.empty(); }
        /// Returns a new read-only buffer over immutable logical bytes.
        @Override public Optional<ByteBuffer> read(String name) throws IOException {
            check(); ResourceIndex.Resource resource = resource(name);
        ...
        @Override public Optional<InputStream> open(String name) throws IOException {
            check(); ResourceIndex.Resource resource = resource(name);
        ...
        @Override public void release(ByteBuffer buffer) { Objects.requireNonNull(buffer); }
        /// Lists all resource names in index order without decoding payloads.
        @Override public Stream<String> list() throws IOException { check(); return root.root.files.keySet().stream().filter(name -> !name.isEmpty()); }
