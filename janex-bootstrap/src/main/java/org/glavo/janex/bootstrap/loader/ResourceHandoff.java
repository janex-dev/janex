...
public final class ResourceHandoff {
    ...
    /// Reads a Host-owned request over verified, unchanged snapshot files and prepares the index.
    /// The stream is consumed and closed; the returned index owns its snapshot handle.
    /// @param stream trusted handoff from the parent that verified the referenced snapshots
    /// @return resource index retaining the container snapshot handle
    /// @throws IOException if handoff framing, resource parsing, or opening the index fails
    public static ResourceIndex read(InputStream stream) throws IOException {
        try (DataInputStream input = new DataInputStream(stream)) {
            if (input.readLong() != 0x4a4e58524f4f5431L) throw new IOException("Invalid root handoff");
    ...
    public static byte[] encode(Path path, String[] context, List<ResourceRequest> requests,
            Map<String, String> requirements, ReadLimits limits, long logicalLimit) throws IOException {
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        DataOutputStream output = new DataOutputStream(bytes);
        output.writeLong(0x4a4e58524f4f5431L);
