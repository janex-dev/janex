...
public final class JanexFileSystemProvider extends FileSystemProvider {
    ...
    /// Creates an unregistered view borrowing the supplied resource snapshot.
    ///
    /// Closing the view does not close the supplied index. The loader owns registration and remounting.
    public FileSystem createView(ResourceIndex index) {
        return new JanexFileSystem(this, Objects.requireNonNull(index));
    }
    ...
    public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
        ...
        return new Channel(target.fileSystem, resource.buffer());
    }
    ...
    private static final class Channel implements SeekableByteChannel {
        ...
        Channel(JanexFileSystem fileSystem, ByteBuffer bytes) {
        ...
        public synchronized int read(ByteBuffer destination) throws IOException {
            ...
            int count = (int) Math.min(destination.remaining(), bytes.limit() - position);
            ByteBuffer part = bytes.duplicate();
            part.position((int) position);
            part.limit((int) position + count);
            destination.put(part);
            position += count;
            return count;
        }
