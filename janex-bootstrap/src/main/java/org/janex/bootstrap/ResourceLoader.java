...
public final class ResourceLoader extends URLClassLoader {
    ...
    public ResourceLoader(ClassLoader parent) throws IOException {
        ...
        try {
            ...
            try { index.close(); } catch (IOException close) { failure.addSuppressed(close); }
            throw failure;
        }
    }
    ...
    @Override public void close() throws IOException {
        try { super.close(); } finally { index.close(); }
    }
    ...
    public static URLConnection connect(URL url) throws IOException {
        ...
        if (loader instanceof ResourceLoader) {
            for (Root root : ((ResourceLoader) loader).roots) {
                try { return root.openConnection(url); }
    ...
    private static final class Root extends URLStreamHandler {
        ...
        Root(ResourceIndex.Root root, int id) throws IOException {
            ...
            if (resource == null) {
                for (Map.Entry<String, ResourceIndex.Resource> entry : root.files.entrySet()) {
                    if (entry.getKey().equalsIgnoreCase("META-INF/MANIFEST.MF")) { resource = entry.getValue(); break; }
                }
            }
        ...
        URL url(String name) {
            try { return new URL(null, new URI("janex", null, prefix + name, null).toASCIIString(), this); }
            catch (URISyntaxException | MalformedURLException invalid) { throw new IllegalArgumentException(invalid); }
        }
        ...
        @Override protected URLConnection openConnection(URL url) throws IOException {
            ...
            return new URLConnection(url) {
                ...
                @Override public InputStream getInputStream() throws IOException { connect(); return new ByteArrayInputStream(resource.read()); }
                /// Returns the declared logical length without decoding the resource.
                @Override public long getContentLengthLong() { return resource.length; }
                /// Returns the declared logical length, which is bounded by Java array limits.
                @Override public int getContentLength() { return resource.length; }
                /// Grants no additional permission merely because a class has a Janex URL.
                @Override public java.security.Permission getPermission() { return null; }
            };
        }
    }
}
