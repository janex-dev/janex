...
public final class JanexFileSystemProvider extends FileSystemProvider {
    ...
    @Override public String getScheme() { return "janex"; }
    ...
    private static void uri(URI uri) {
        if (!"janex".equalsIgnoreCase(uri.getScheme()) || uri.getRawAuthority() != null
                ...
                || uri.getPath() == null || !uri.getPath().startsWith("/")) throw new IllegalArgumentException("Invalid Janex URI: " + uri);
    }
    ...
    @Override public FileSystem newFileSystem(URI uri, Map<String, ?> env) {
        uri(uri); if (!Objects.requireNonNull(env).isEmpty()) throw new IllegalArgumentException("No Janex mount options");
        return ResourceLoader.fileSystem(this, true);
    }
    /// Returns the open launch view, creating its initial mount lazily.
    @Override public FileSystem getFileSystem(URI uri) { uri(uri); return ResourceLoader.fileSystem(this, false); }
    /// Returns the path corresponding to a class-loader or module-reader resource URI.
    @Override public Path getPath(URI uri) { return getFileSystem(uri).getPath(uri.getPath()); }
    /// Requires a Janex path without confusing paths from other providers.
    private static JanexPath path(Path path) {
        Objects.requireNonNull(path);
        if (!(path instanceof JanexPath)) throw new ProviderMismatchException();
        return (JanexPath) path;
    }
    /// Checks link options; the snapshot contains resolved aliases rather than live links.
    static void links(LinkOption... options) { for (LinkOption option : options) Objects.requireNonNull(option); }
    ...
    @Override public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
        ...
        for (OpenOption option : options) {
            ...
            if (option == StandardOpenOption.READ || option == LinkOption.NOFOLLOW_LINKS) continue;
            if (option instanceof StandardOpenOption) throw new ReadOnlyFileSystemException();
            throw new UnsupportedOperationException("Unsupported open option: " + option);
        }
        if (attrs.length != 0) throw new UnsupportedOperationException("Initial attributes on read-only channel");
        ResourceIndex.Resource resource = target.fileSystem.node(target);
        if (resource == null || resource.id == -1) throw new FileSystemException(path.toString(), null, "Is a directory");
    ...
    @Override public void createDirectory(Path dir, FileAttribute<?>... attrs) { path(dir).fileSystem.ensureOpen(); throw new ReadOnlyFileSystemException(); }
    /// Rejects deletion in the immutable snapshot.
    @Override public void delete(Path path) { path(path).fileSystem.ensureOpen(); throw new ReadOnlyFileSystemException(); }
    /// Rejects a copy whose destination belongs to this read-only provider.
    @Override public void copy(Path source, Path target, CopyOption... options) { path(source).fileSystem.ensureOpen(); path(target).fileSystem.ensureOpen(); throw new ReadOnlyFileSystemException(); }
    /// Rejects moving entries in the immutable snapshot.
    @Override public void move(Path source, Path target, CopyOption... options) { copy(source, target, options); }
    /// Returns true for equal paths; otherwise compares existing normalized paths within the same view.
    @Override public boolean isSameFile(Path first, Path second) throws IOException {
        JanexPath left = path(first);
        if (left.equals(second)) return true;
        if (!(second instanceof JanexPath) || second.getFileSystem() != left.fileSystem) return false;
        return left.toRealPath().equals(second.toRealPath());
    }
    /// Returns false for an existing entry; the format has no hidden-file flag.
    @Override public boolean isHidden(Path path) throws IOException { JanexPath target = path(path); target.fileSystem.node(target); return false; }
    /// Returns the logical read-only store containing an existing entry.
    @Override public FileStore getFileStore(Path path) throws IOException { JanexPath target = path(path); target.fileSystem.node(target); return new Store(target.fileSystem); }
    ...
    @Override public void checkAccess(Path path, AccessMode... modes) throws IOException {
        JanexPath target = path(path); target.fileSystem.node(target);
        for (AccessMode mode : modes) if (Objects.requireNonNull(mode) != AccessMode.READ) throw new AccessDeniedException(path.toString());
    }
    /// Returns a basic attribute view, or null when the requested view type is unsupported.
    @Override public <V extends FileAttributeView> V getFileAttributeView(final Path path, Class<V> type, LinkOption... options) {
        path(path); Objects.requireNonNull(type); links(options);
        ...
        return type.cast(new BasicFileAttributeView() {
            ...
            @Override public BasicFileAttributes readAttributes() throws IOException { return JanexFileSystemProvider.this.readAttributes(path, BasicFileAttributes.class); }
            /// Rejects timestamp mutation.
            @Override public void setTimes(FileTime modified, FileTime accessed, FileTime created) { path(path).fileSystem.ensureOpen(); throw new ReadOnlyFileSystemException(); }
        });
    }
    ...
    @Override public <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> type, LinkOption... options) throws IOException {
        JanexPath target = path(path); Objects.requireNonNull(type); links(options);
        if (type != BasicFileAttributes.class) throw new UnsupportedOperationException("Unsupported attribute type: " + type);
    ...
    @Override public Map<String, Object> readAttributes(Path path, String attributes, LinkOption... options) throws IOException {
        ...
        if (!view.equals("basic") && !view.equals("janex")) throw new UnsupportedOperationException("Unsupported attribute view: " + view);
        ...
        all.put("size", value.size()); all.put("creationTime", value.creationTime());
        all.put("lastModifiedTime", value.lastModifiedTime()); all.put("lastAccessTime", value.lastAccessTime());
        all.put("isRegularFile", value.isRegularFile()); all.put("isDirectory", value.isDirectory());
        all.put("isSymbolicLink", false); all.put("isOther", false); all.put("fileKey", value.fileKey());
        if (view.equals("janex")) {
            all.put("creationTimeNanos", value.nanos(0)); all.put("lastModifiedTimeNanos", value.nanos(1));
        ...
        for (String name : names.split(",", -1)) {
            if (!all.containsKey(name)) throw new IllegalArgumentException("Unknown attribute: " + name);
            selected.put(name, all.get(name));
        }
        return selected;
    }
    /// Rejects attribute mutation.
    @Override public void setAttribute(Path path, String attribute, Object value, LinkOption... options) { path(path).fileSystem.ensureOpen(); links(options); throw new ReadOnlyFileSystemException(); }
    ...
    private static final class Channel implements SeekableByteChannel {
        ...
        Channel(JanexFileSystem fileSystem, byte[] bytes) { this.fileSystem = fileSystem; this.bytes = bytes; }
        /// Requires an open channel and view.
        private void check() throws ClosedChannelException { if (bytes == null || !fileSystem.isOpen()) throw new ClosedChannelException(); }
        /// Reads into the destination, advancing both positions; an empty destination returns zero even at EOF.
        @Override public synchronized int read(ByteBuffer destination) throws IOException {
            check(); Objects.requireNonNull(destination);
            ...
            destination.put(bytes, (int) position, count); position += count; return count;
        }
        /// Rejects writes without consuming the source.
        @Override public synchronized int write(ByteBuffer source) throws IOException { check(); throw new NonWritableChannelException(); }
        /// Returns the current offset.
        @Override public synchronized long position() throws IOException { check(); return position; }
        /// Sets a nonnegative offset, including offsets beyond EOF.
        @Override public synchronized SeekableByteChannel position(long value) throws IOException { check(); if (value < 0) throw new IllegalArgumentException("Negative position"); position = value; return this; }
        /// Returns the immutable decoded length.
        @Override public synchronized long size() throws IOException { check(); return bytes.length; }
        /// Rejects truncation.
        @Override public synchronized SeekableByteChannel truncate(long size) throws IOException { check(); if (size < 0) throw new IllegalArgumentException("Negative size"); throw new NonWritableChannelException(); }
        /// Tests both channel and view lifecycle.
        @Override public synchronized boolean isOpen() { return bytes != null && fileSystem.isOpen(); }
    ...
    private static final class Listing implements DirectoryStream<Path>, Iterator<Path> {
        ...
        Listing(JanexPath directory, List<String> names, Filter<? super Path> filter) { this.directory = directory; this.names = names.iterator(); this.filter = filter; }
        /// Returns the sole iterator; closed or previously iterated listings reject this call.
        @Override public synchronized Iterator<Path> iterator() { if (closed || iterated) throw new IllegalStateException("Directory stream is closed or already iterated"); iterated = true; return this; }
        ...
        @Override public synchronized boolean hasNext() {
            ...
            while (next == null && names.hasNext()) {
                ...
                try { if (filter.accept(candidate)) next = candidate; }
                catch (IOException failure) { throw new DirectoryIteratorException(failure); }
            }
            return next != null;
        }
        /// Returns the next accepted entry, including an entry prefetched before closure.
        @Override public synchronized Path next() { if (!hasNext()) throw new NoSuchElementException(); Path result = next; next = null; return result; }
        /// Rejects entry removal.
        @Override public void remove() { throw new UnsupportedOperationException("Read-only directory"); }
    ...
    private static final class Attributes implements BasicFileAttributes {
        ...
        Attributes(JanexPath path, ResourceIndex.Resource resource) { this.path = path.toAbsolutePath().normalize(); this.resource = resource; }
        /// Returns a nullable exact timestamp in nanoseconds.
        BigInteger nanos(int index) { return resource == null ? null : resource.times[index]; }
        ...
        private FileTime time(int index) {
            ...
            if (value == null) return FileTime.fromMillis(0);
            ...
            if (parts[0].bitLength() <= 63) {
                ...
                try { return FileTime.from(Instant.ofEpochSecond(seconds, parts[1].longValue())); }
                catch (java.time.DateTimeException outsideInstant) { return FileTime.from(seconds, TimeUnit.SECONDS); }
            }
        ...
        @Override public FileTime lastModifiedTime() { return time(1); }
        /// Returns the stored access time or the epoch.
        @Override public FileTime lastAccessTime() { return time(2); }
        /// Returns the stored creation time or the epoch.
        @Override public FileTime creationTime() { return time(0); }
        /// Tests whether this entry has file content.
        @Override public boolean isRegularFile() { return resource != null && resource.id != -1; }
        /// Tests whether this entry is an explicit or synthetic directory.
        @Override public boolean isDirectory() { return !isRegularFile(); }
        ...
        @Override public long size() { return resource == null ? 0 : resource.length; }
        /// Returns the normalized path as a view-scoped identity.
        @Override public Object fileKey() { return path; }
    }
    ...
    static final class Store extends FileStore {
        ...
        Store(JanexFileSystem fileSystem) { this.fileSystem = fileSystem; }
        /// Returns the logical snapshot name.
        @Override public String name() { return "janex"; }
        /// Returns the archive store type.
        @Override public String type() { return "janex"; }
        /// Returns true.
        @Override public boolean isReadOnly() { return true; }
        /// Returns the sum of decoded resource lengths without decoding payloads.
        @Override public long getTotalSpace() { fileSystem.ensureOpen(); long size = 0; for (ResourceIndex.Resource resource : fileSystem.entries.values()) if (resource != null) size += resource.length; return size; }
        /// Returns zero because the snapshot has no writable space.
        @Override public long getUsableSpace() { fileSystem.ensureOpen(); return 0; }
        /// Returns zero because the snapshot has no allocation space.
        @Override public long getUnallocatedSpace() { fileSystem.ensureOpen(); return 0; }
        /// Tests support for the standard basic attribute view.
        @Override public boolean supportsFileAttributeView(Class<? extends FileAttributeView> type) { return Objects.requireNonNull(type) == BasicFileAttributeView.class; }
        /// Tests support for basic or Janex metadata attributes.
        @Override public boolean supportsFileAttributeView(String name) { return fileSystem.supportedFileAttributeViews().contains(Objects.requireNonNull(name)); }
        ...
        @Override public Object getAttribute(String name) { if (name.equals("totalSpace")) return getTotalSpace(); if (name.equals("usableSpace")) return getUsableSpace(); if (name.equals("unallocatedSpace")) return getUnallocatedSpace(); throw new UnsupportedOperationException(name); }
    }
}
