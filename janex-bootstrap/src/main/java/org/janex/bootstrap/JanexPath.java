...
final class JanexPath implements Path {
    ...
    JanexPath(JanexFileSystem fileSystem, String value) {
        if (value.indexOf('\0') >= 0) {
            throw new InvalidPathException(value, "NUL in path");
        }
    ...
    private JanexPath same(Path other) {
        ...
        if (!(other instanceof JanexPath) || other.getFileSystem() != fileSystem) {
    ...
    @Override
    public JanexFileSystem getFileSystem() {
        return fileSystem;
    }
    ...
    public Path getFileName() {
        return value.equals("/") ? null : fileSystem.getPath(value.substring(value.lastIndexOf('/') + 1));
    }
    ...
    public Path subpath(int begin, int end) {
        String[] names = names();
        if (begin < 0 || end > names.length || begin >= end) {
    ...
    public boolean startsWith(Path other) {
        if (!(other instanceof JanexPath) || other.getFileSystem() != fileSystem || isAbsolute() != other.isAbsolute()) {
            return false;
        }
    ...
    @Override
    public JanexPath normalize() {
        List<String> result = new ArrayList<String>();
        for (String name : names()) {
            if (name.isEmpty() || name.equals(".")) {
                continue;
            }
            if (name.equals("..") && !result.isEmpty() && !result.get(result.size() - 1).equals("..")) {
    ...
    public Path resolve(Path other) {
        ...
        if (path.isAbsolute() || value.isEmpty()) {
            return path;
        }
    ...
    public Path relativize(Path other) {
        ...
        if (base.isAbsolute() != target.isAbsolute()) {
            throw new IllegalArgumentException("Different path kinds");
        }
        if (base.value.isEmpty()) {
            return target;
        }
        String[] left = base.names();
        String[] right = target.value.isEmpty() ? new String[0] : target.names();
        int common = 0;
        while (common < left.length && common < right.length && left[common].equals(right[common])) {
            common++;
        }
        List<String> parts = new ArrayList<String>();
        for (int i = common; i < left.length; i++) {
            if (left[i].equals("..")) {
                throw new IllegalArgumentException("Unresolved parent in base path");
            }
            parts.add("..");
        }
    ...
    public URI toUri() {
        try {
            return new URI("janex", null, toAbsolutePath().toString(), null);
    ...
    @Override
    public JanexPath toAbsolutePath() {
    ...
    @Override
    public File toFile() {
        throw new UnsupportedOperationException("Janex paths are not native files");
    }
    ...
    public Iterator<Path> iterator() {
        ...
        for (String name : names()) {
            paths.add(fileSystem.getPath(name));
        }
    ...
    public int compareTo(Path other) {
        Objects.requireNonNull(other);
        if (!(other instanceof JanexPath) || other.getFileSystem() != fileSystem) {
            throw new ClassCastException("Different file system");
        }
    ...
    public boolean equals(Object other) {
        return other instanceof JanexPath && ((JanexPath) other).fileSystem == fileSystem && value.equals(((JanexPath) other).value);
    }

    /// Hashes file-system identity and lexical representation.
    @Override
    public int hashCode() {
        return 31 * System.identityHashCode(fileSystem) + value.hashCode();
    }
