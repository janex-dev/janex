...
import org.janex.bootstrap.fs.JanexFileSystemProvider;
...
public final class ResourceLoader extends URLClassLoader {
    ...
    public ResourceLoader(ClassLoader parent) throws IOException {
        ...
        InputStream data = ResourceLoader.class.getResourceAsStream("/org/janex/bootstrap/resources.bin");
        if (data == null) {
        ...
        try {
            ...
            try {
                Class.forName("org.janex.bootstrap.loader.ModuleSupport").getMethod("initialize", ResourceLoader.class).invoke(null, this);
    ...
    ///
    /// @param provider provider used when creating a view
    /// @param create whether to request a new view, rejecting an existing open view
    /// @return the loader's mounted view, borrowing its snapshot
    /// @throws java.nio.file.FileSystemNotFoundException if no active snapshot exists, or a closed
    ///         view is requested without creating a replacement
    /// @throws java.nio.file.FileSystemAlreadyExistsException if create is true and a view is open
    public static FileSystem fileSystem(JanexFileSystemProvider provider, boolean create) {
        ...
        synchronized (loader) {
            if (loader.fileSystem == null) {
                loader.fileSystem = provider.createView(loader.index);
            } else if (create) {
